...
protected:
  ...
  /** Advances @a iter, calling @a predicate on each character. If
   *  @a predicate returns <tt>true</tt>, returns <tt>true</tt> and stops scanning.
   * If @a predicate never returns <tt>true</tt>, @a iter is set to @a limit if
   *  @a limit is non-<tt>nullptr</tt>, otherwise to the end iterator.
   * 
   * @param predicate A function to be called on each character.
   * @param user_data User data for @a predicate.
   * @param limit Search limit, or <tt>nullptr</tt> for none.
...
public:
   ...
   * is <tt>nullptr</tt>, returns <tt>true</tt> if any tag is toggled on at this point.
   * 
   * Note that if starts_tag() returns <tt>true</tt>, it means that @a iter is
   ...
   * @newin{3,20}
   * 
   * @param tag A Gtk::TextTag, or <tt>nullptr</tt>.
   ...
   * is <tt>nullptr</tt>, returns <tt>true</tt> if any tag is toggled off at this point.
   * 
   ...
   * unlike starts_tag(), if ends_tag() returns <tt>true</tt>,
   ...
   * with @a tag. See also starts_tag() and ends_tag().
   * 
   ...
   *  @a tag is <tt>nullptr</tt>. If no matching tag toggles are found,
