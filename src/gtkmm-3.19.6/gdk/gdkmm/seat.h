...
public:
  typedef Seat CppObjectType;
  typedef Seat_Class CppClassType;
  typedef GdkSeat BaseObjectType;
  typedef GdkSeatClass BaseClassType;
...
public:
   ...
   * grab on the window with @a owner_events set to <tt>true</tt>. This performed as most
   ...
   * is available.
   * @param slot Function to prepare the window to be
   * grabbed, it can be <tt>nullptr</tt> if @a window is visible before this call.
   ...
   * 
   * @return A Gdk::Display. This memory is owned by GTK+ and
   * must not be freed.
   */
...
//#m4 __CONVERSION(`GList*',`std::vector<Glib::RefPtr<Device>>',`Glib::ListHandler<Glib::RefPtr<Device>>::list_to_vector($3, Glib::OWNERSHIP_SHALLOW)')
//  _WRAP_METHOD(std::vector<Glib::RefPtr<Device>> get_slaves(SeatCapabilities capabilities), gdk_seat_get_slaves)
//#m4 __CONVERSION(`GList*',`std::vector<Glib::RefPtr<const Device>>',`Glib::ListHandler<Glib::RefPtr<const Device>>::list_to_vector($3, Glib::OWNERSHIP_SHALLOW)')
//  _WRAP_METHOD(std::vector<Glib::RefPtr<const Device>> get_slaves(SeatCapabilities capabilities) const, gdk_seat_get_slaves)
   ...
   *
   * The signal_device_added() signal is emitted either when a new input
  ...
  Glib::SignalProxy1< void,const Glib::RefPtr<Device>& > signal_device_added();
   ...
   *
   * The signal_device_removed() signal is emitted either when an
  ...
  Glib::SignalProxy1< void,const Glib::RefPtr<Device>& > signal_device_removed();
