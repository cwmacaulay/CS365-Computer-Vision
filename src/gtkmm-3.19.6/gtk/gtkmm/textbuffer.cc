...
{
  return gtk_text_buffer_serialize(gobj(), Glib::unwrap(content_buffer), Gdk::AtomString::to_c_type(format), (range_start).gobj(), (range_end).gobj(), &(length));
}
...
bool TextBuffer::deserialize(const Glib::RefPtr<TextBuffer>& content_buffer, const Glib::ustring& format, iterator& iter, const guint8* data, gsize length)
{
  ...
  bool retvalue = gtk_text_buffer_deserialize(gobj(), Glib::unwrap(content_buffer), Gdk::AtomString::to_c_type(format), (iter).gobj(), data, length, &(gerror));
  if(gerror)
